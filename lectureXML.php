...
//                    $sLinks .= "{from: 'grp_" . $oGroup['id'] . "', to: 'ele_" . $sRefIdDoc . "'},\n";
   ...
   // var_dump( $oRefDocsGrps );
    ...
    for ($i = 0; $i < $xml->document->count(); $i++) {
        ...
        $sTitle .= '"ele_'.$oDoc['id'].'":"'.$oDoc->title.'",';
    }
    $sUrls = rtrim(trim($sUrls), ',');
    $sTitle = rtrim(trim($sTitle), ',');
...
return;
                ...
                document.getElementById('eventSpan').innerHTML = '<br> <h2>Titre:</h2> <p>' + oTitle [params.nodes[0] ] + '</p><h2>Lien:</h2> <a href="' + oUrls [params.nodes[0] ] + '" target="blank">' + oUrls [params.nodes[0] ] +'</a>';
